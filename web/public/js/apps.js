...
$('#modeSwitcher').on('click', function (e) {
	...
	$('.collapseSidebar').on('click', function (e) {
		$('.vertical').hasClass('narrow')
			...
			: ($('.vertical').toggleClass('collapsed'), $('.vertical').hasClass('hover') && $('.vertical').removeClass('hover')),
			e.preventDefault();
	}),
	$('.sidebar-left').hover(
		function () {
	...
	$('.close-info').on('click', function () {
		$('.file-container').hasClass('collapsed') &&
			($('.file-container').removeClass('collapsed'), $('.file-panel').find('.selected').removeClass('selected'));
	}),
	$(function () {
...
vertical_wizard.length &&
	vertical_wizard.steps({ headerTag: 'h3', bodyTag: 'section', transitionEffect: 'slideLeft', stepsOrientation: 'vertical' });
